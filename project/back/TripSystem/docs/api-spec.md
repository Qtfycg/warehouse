...
## 1. 接口清单
### 1.1 用户相关接口
#### 1.1.1 POST /api/user/login`: 用户登录
```text
1. 请求参数
- `phone` (string, 必填): 用户手机号
- `password` (string, 必填): 密码(加密后储存)
- `captcha` (string, 必填): 验证码
2. 响应参数

```
### 1.1.2 `GET /api/user/me`: 获取用户信息
```text

```
### 1.1.3 `PUT /api/user/update`: 更新用户信息
```text

```
### 1.1.4 `GET /api/user/captcha`: 生成验证码
```text

```
### 1.1.5 `POST /api/user/register`: 用户注册
```text
